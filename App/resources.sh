...
for file in $(find ${PWD} -not -path "*/Pods/*" -not -path "*/preview/*" -name "*.swift"); do
...
contents=""
for file in $(find ${PWD} -not -path "*/Pods/*" -not -path "*/preview/*" -name "*.swift"); do
for name in $(cat $file | grep "R\.event\." | grep -v "func" | grep -v "R\.event\.[a-zA-Z0-9_\-]*\."| sed 's/.*R\.event\.\([a-zA-Z0-9_\-]*\).* as \([a-zA-Z0-9_\-]*\).*/\1\(Any)/' | sed 's/.*R\.event\.\([a-zA-Z0-9_\-]*\).*/\1/'); do
contents=$contents" $name"
done
done
...
# find assets
START_TIME="$(date +%s)"
contents=$(find ${PWD} -not -path "*/Pods/*" -not -path "*/preview/*" -type d -name "*.imageset" | sed 's!.*/!!' | sed 's/\.[^.]*$//')
...
START_TIME="$(date +%s)"
contents=""
for file in $(find ${PWD} -not -path "*/Pods/*" -not -path "*/preview/*" -not -path "*/external/*" -name "*.strings"); do
...
echo $(cat $tmpfile) >> $R
echo "extension Dictionary where Iterator.Element == (key: String, value: Any) { func extend(_ ext : [String:Any]) -> Dictionary<String,Any> { return self.merging(ext) { (_, new) in new } } }" >> $R
